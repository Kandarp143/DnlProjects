...
<jsp:useBean id="objK" class="kp.beans.data.pojo.MocQcMst" scope="request">
    <jsp:setProperty property="*" name="objK"/>
</jsp:useBean>  
<%
    if (errors.size() > 0) {
        request.setAttribute("errors", errors);
%>
<jsp:forward page="../retryQc.jsp"></jsp:forward>
<%
    } else {
        objK.setMocInitMst(mst);
        objK.setUserId(session.getAttribute("usr").toString());
        if (d.fecthQc(cid) == null) {
            cid = d.saveQc(objK);
        } else {
            d.updateQc(objK);
        }
