...
public class Login extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if (userValidate.equals("SUCCESS")) {
            ...
            int seq = g.Get_Seq("SEQ", "CBA_LOGIN_HISTORY");
            loginBean.setSEQ(seq);
            loginBean.setUSER_ID(request.getParameter("login_id"));
            loginDao.addLoginHistory(loginBean);
            LoginBean loginBean2 = new LoginBean();
            loginBean2 = loginDao.getUserDetail(loginBean);
            session.setAttribute("seq", seq);
            session.setAttribute("uid", request.getParameter("login_id"));
            session.setAttribute("uname", loginBean2.getUSER_NAME());
            session.setAttribute("unit", loginBean2.getUNIT());
